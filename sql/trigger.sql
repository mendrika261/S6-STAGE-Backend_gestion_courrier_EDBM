...
AS $$
BEGIN
    INSERT INTO mail_history (id, created_at, description, mail_confidentiality, note_for_messenger, object, priority, receiver, reference, sender, status, created_by_id, mail_id, receiver_location_id, sender_location_id)
    VALUES (nextval('mail_history_seq'), now(), NEW.description, NEW.confidentiality, NEW.note_for_messenger, NEW.object, NEW.priority, NEW.receiver, NEW.reference, NEW.sender, NEW.status, NEW.created_by_id, NEW.id, NEW.receiver_location_id, NEW.sender_location_id);
