...
public class MouvementAnalysis extends Analysis {
    ...
    public List<Input> getPossibleColumns() {
        return List.of(
                ...
                new Input("Date de début - Mois", "TO_CHAR(m.start_date, 'TMMonth')"),
                new Input("Date de début - Année", "extract('year' from m.start_date)"),
                new Input("Date de début - Jour de la semaine", "TO_CHAR(m.start_date, 'TMDay')"),
                ...
                new Input("Date de fin - Mois", "TO_CHAR(m.end_date, 'TMMonth')"),
                new Input("Date de fin - Année", "extract('year' from m.end_date)"),
                new Input("Date de fin - Jour de la semaine", "TO_CHAR(m.end_date, 'TMDay')"),
                new Input("Expéditeur", "m.sender"),
                new Input("Localisation de l'expéditeur", "ls.name, ls.latitude, ls.longitude"),
                new Input("Destinataire", "m.receiver"),
                new Input("Localisation du destinataire", "lr.name, lr.latitude, lr.longitude"),
    ...
    public List<Input> getPossibleMeasures() {
        return List.of(
                new Input("Nombre de mouvements", "count(distinct m.id)"),
                new Input("Moyenne de durée (m)", "EXTRACT(EPOCH FROM avg(m.end_date - m.start_date)) / 60"),
                new Input("Durée médiane (m)", "EXTRACT(EPOCH FROM percentile_cont(0.5) within group (order by m.end_date - m.start_date)) / 60"),
                new Input("Durée minimale (m)", "EXTRACT(EPOCH FROM min(m.end_date - m.start_date)) / 60"),
                new Input("Durée maximale (m)", "EXTRACT(EPOCH FROM max(m.end_date - m.start_date)) / 60"),
                new Input("Durée totale (m)", "EXTRACT(EPOCH FROM sum(m.end_date - m.start_date)) / 60")
        );
    }
